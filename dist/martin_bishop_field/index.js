...
function sendConfirmationEmail(env, r) {
  const date = (/* @__PURE__ */ new Date(r.event_date + "T12:00:00")).toLocaleDateString("en-US", { weekday: "long", month: "long", day: "numeric", year: "numeric" });
  const checkInfo = r.payment_type === "check" ? `<p><strong>Check Payment Instructions:</strong><br>Please mail a $500 deposit check within 5 business days.<br>Make payable to: Martin Bishop Field<br>Mail to: 225 State Street, Guilford, CT 06437</p>` : "";
  const paymentStatus = r.amount_paid >= r.amount_total ? "Paid in full" : r.payment_type === "check" ? "Check payment pending" : `$${(r.amount_paid / 100).toFixed(2)} paid — balance of $${((r.amount_total - r.amount_paid) / 100).toFixed(2)} due within 30 days of event`;
...
function sendCancellationEmail(env, r, refundInfo) {
  const date = (/* @__PURE__ */ new Date(r.event_date + "T12:00:00")).toLocaleDateString("en-US", { weekday: "long", month: "long", day: "numeric", year: "numeric" });
  return sendEmail(env, {
    to: [r.email, FIELD_EMAIL],
...
function sendInvoiceEmail(env, r) {
  const date = (/* @__PURE__ */ new Date(r.event_date + "T12:00:00")).toLocaleDateString("en-US", { weekday: "long", month: "long", day: "numeric", year: "numeric" });
...
function sendConcernEmail(env, r) {
  const date = (/* @__PURE__ */ new Date(r.event_date + "T12:00:00")).toLocaleDateString("en-US", { weekday: "long", month: "long", day: "numeric", year: "numeric" });
  const balance = ((r.amount_total - r.amount_paid) / 100).toFixed(2);
  return sendEmail(env, {
    to: [FIELD_EMAIL],
...
async function handleRequest(request, env) {
  ...
  try {
    ...
    if (actionMatch && method === "POST") {
      ...
      switch (action) {
        case "cancel":
          return cancelReservation(request, env, id);
        case "refund":
          return refundReservation(request, env, id);
        case "invoice":
          return sendInvoice(request, env, id);
        case "mark-paid":
          return markPaid(request, env, id);
      }
    }
...
async function createCheckReservation(request, env) {
  const body = await request.json();
...
async function stripeWebhook(request, env) {
  ...
  if (event.type === "payment_intent.succeeded") {
    ...
    if (reservation) {
      ...
      const paidInFull = amountPaid >= reservation.amount_total ? 1 : 0;
...
async function addReservation(request, env) {
  const auth = await requireAuth(request, env);
  if (auth instanceof Response) return auth;
  const body = await request.json();
  const result = await env.DB.prepare(
...
async function refundReservation(request, env, id) {
  const auth = await requireAuth(request, env);
  if (auth instanceof Response) return auth;
  const reservation = await env.DB.prepare(`SELECT * FROM reservations WHERE id = ?`).bind(id).first();
  if (!reservation) return errorResponse("Reservation not found", 404);
  ...
  const refundAmount = reservation.amount_total - DEPOSIT_AMOUNT;
...
async function sendInvoice(request, env, id) {
  const auth = await requireAuth(request, env);
  if (auth instanceof Response) return auth;
  const reservation = await env.DB.prepare(`SELECT * FROM reservations WHERE id = ?`).bind(id).first();
  if (!reservation) return errorResponse("Reservation not found", 404);
...
async function markPaid(request, env, id) {
  const auth = await requireAuth(request, env);
  if (auth instanceof Response) return auth;
  const reservation = await env.DB.prepare(`SELECT * FROM reservations WHERE id = ?`).bind(id).first();
  if (!reservation) return errorResponse("Reservation not found", 404);
...
async function listUsers(request, env) {
  const auth = await requireAuth(request, env);
  if (auth instanceof Response) return auth;
  const { results } = await env.DB.prepare(`SELECT id, username FROM admins ORDER BY id`).all();
...
async function deleteUser(request, env, id) {
  const auth = await requireAuth(request, env);
  if (auth instanceof Response) return auth;
  const count = await env.DB.prepare(`SELECT COUNT(*) as cnt FROM admins`).first();
  if (count && count.cnt <= 1) return errorResponse("Cannot delete the last admin user");
  await env.DB.prepare(`DELETE FROM admins WHERE id = ?`).bind(id).run();
  return jsonResponse({ success: true });
}
async function updateUserPassword(request, env, id) {
  const auth = await requireAuth(request, env);
  if (auth instanceof Response) return auth;
...
async function verifyStripeSignature(payload, sigHeader, secret) {
  ...
  if (Math.abs(Date.now() / 1e3 - parseInt(timestamp)) > 300) return null;
  const encoder = new TextEncoder();
  const key = await crypto.subtle.importKey("raw", encoder.encode(secret), { name: "HMAC", hash: "SHA-256" }, false, ["sign"]);
